...
const App: React.FC = () => {
    ...
    const handleSaveApiKey = (key: string) => {
        ...
        if (pendingAiAction) {
            // Use a timeout to ensure the state update has propagated before running the action
            setTimeout(() => {
                pendingAiAction();
    ...
    const runAiAction = (action: (key: string) => void) => {
        if (apiKey) {
            action(apiKey);
        } else {
            // Set the action to be run after the key is provided
            setPendingAiAction(() => () => action(localStorage.getItem('geminiApiKey')!));
            setIsApiKeyModalOpen(true);
        }
    };

    const handleGenerateConfig = () => {
        runAiAction(async (key) => {
            ...
            try {
                const configJsonString = await generateConfigFromHtml(allHtmlContent, key);
                ...
                const newConfigFile: StoredFile = { name: '_config.json', content: prettyJson, type: 'application/json' };
                setAllUploadedFiles(prevMap => new Map(prevMap).set('_config.json', newConfigFile));
                
                setConfigHighlighted(true);
                setTimeout(() => setConfigHighlighted(false), 1500);
            } catch (e: any) {
                console.error("Failed to generate or parse config:", e);
                const friendlyMessage = e.message.includes("API key not valid")
                    ? "Неверный API ключ. Пожалуйста, проверьте его и попробуйте снова."
                    : "Не удалось сгенерировать файл конфигурации. Убедитесь, что в ваших HTML-файлах есть явные плейсхолдеры.";
    ...
    const handleOptimizeNames = () => {
        runAiAction(async (key) => {
            ...
            try {
                ...
                const hasApiError = newNames.some(n => n.newName.includes("ошибка-ключ-api"));
                if (hasApiError) {
                ...
                setTimeout(() => setHighlightedItems({}), 1500);
                 ...
                 const friendlyMessage = err.message.includes("API key not valid")
                    ? "Неверный API ключ. Пожалуйста, проверьте его и попробуйте снова."
                    : "Произошла ошибка при оптимизации имен файлов.";
                setError(friendlyMessage);
            } finally {
                setIsLoading(false);
            }
